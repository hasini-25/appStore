...
class AppStore extends Component {
  state = {searchInput: '', activeTabItem: tabsList[0].tabId}
  ...
  getFilteredAppStore = searchApp => {
    const {activeTabItem} = this.state
    const appStoreFilter = searchApp.filter(
      eachAppStore => eachAppStore.category === activeTabItem,
    )
  ...
  filterSearchResult = () => {
    ...
    const filterApp = appsList.filter(eachApp =>
  ...
  render() {
    const {searchInput, activeTabItem} = this.state
    const filterResult = this.filterSearchResult()
    const appStoreFilter = this.getFilteredAppStore(filterResult)
        ...
        <div className="container">
          ...
          <ul className="tab-items">
            {tabsList.map(eachItem => (
              <TabItem
                onClickTabItems={this.onClickTabItems}
                key={eachItem.tabId}
                tabsList={eachItem}
                isActive={eachItem.tabId === activeTabItem}
              />
            ))}
          </ul>
          <ul className="app-store">
            {appStoreFilter.map(eachApp => (
